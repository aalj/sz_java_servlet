...
public class ScoreStat {
	...
	public static  int stat(List<Student> students,String courseName,int[]arr) {
		int  a = 0;
		//60,70,80,90,
		//返回统计出的人数
		//数组里面
		Map<Student,Double> scores =  getStudentTotalScore(students,courseName);
		Set<Student> keySet = scores.keySet();
		for (Student i : keySet) {
			if (scores.get(i)>arr[0]&&scores.get(i)<arr[1]) {
				a+=1;
			}
